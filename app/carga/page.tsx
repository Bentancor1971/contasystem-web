...
export default function CargaPage() {
  ...
  const [tab, setTab] = useState<'cargar' | 'ultimos'>('cargar')
  ...
  // ── Bootstrap: carga empresa elegida + catálogos + últimos 20 ────────────
  useEffect(() => {
    async function load() {
  ...
  // Pre-llenar descripción al elegir plantilla
  useEffect(() => {
    if (plantillaSeleccionada && !descripcion) {
      setDescripcion(plantillaSeleccionada.descripcion_default ?? '')
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [plantillaSeleccionada])

  // Mapas para mostrar nombres en la lista
  ...
  return (
    <>
      ...
      {/* Tabs mobile */}
      <div className="md:hidden bg-white border-b border-line px-5">
        <div className="flex">
          <button className="tab" aria-selected={tab === 'cargar'} onClick={() => setTab('cargar')}>Cargar</button>
      ...
      <main className="max-w-7xl mx-auto px-5 md:px-8 lg:px-10 py-7 lg:py-12 flex-1">
        <div className="grid lg:grid-cols-12 gap-6 lg:gap-8">

          {/* ── FORM ────────────────────────────────────── */}
          <section className={`lg:col-span-5 ${tab === 'ultimos' ? 'hidden md:block' : ''}`}>
            <form onSubmit={guardar} className="card p-6 lg:p-8 rise">
          ...
          {/* ── ÚLTIMOS 20 ─────────────────────────────── */}
          <section className={`lg:col-span-7 ${tab === 'cargar' ? 'hidden md:block' : ''}`}>
            <div className="card p-6 lg:p-8">
              ...
              {comprobantes.length === 0 ? (
                <div className="py-12 text-center">
                  ...
                  <p className="text-sm text-ink-3">Cargá tu primer comprobante a la izquierda.</p>
                </div>
              ) : (
                <div>
